...
namespace E_Commerce.Infrastructure.Repositries
{
    internal class GenericRepositriy<TEntity, Tkey>(StoreDbContext dbContext) : IGenericRepository<TEntity, Tkey> where TEntity : BaseEntity<Tkey>
    {
       ...
       => dbContext.Set<TEntity>().Remove(Entity);
        ...
        public async Task<IEnumerable<TEntity>> GetAll(ISpecification<TEntity, Tkey> spec, CancellationToken ct)
        {
            IQueryable<TEntity> query =  SpecificationEvallutor.CreateQuery(dbContext.Set<TEntity>(), spec); 
        ...
        public async Task<TEntity> GetById(ISpecification<TEntity, Tkey> spec, CancellationToken ct)
        {
            IQueryable<TEntity> query = SpecificationEvallutor.CreateQuery(dbContext.Set<TEntity>(), spec);
